...
function PropertyPage({p}:{p:Property}){
  const reserve=async(date:string,time:string)=>{const r=await createCheckoutSession(p.id,date,time);if(r.url){window.location.href=r.url;return}if(r.demo){go(r.demo);return}throw new Error("Checkout is not available on this deployment.")};
